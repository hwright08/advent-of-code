...
function parseData(d) {
  let line, pass, rules, letter;
  let newData = [];
  for (let i in d) {
    line = data[i].split(' ');
    rules = line[0].split('-');
    letter = line[1].slice(0,1);
    pass = line[2];

    newData.push({
      lower: parseInt(rules[0]),
      upper: parseInt(rules[1]),
      letter,
      pass
    });
  }
...
for (let i in parsedData) {
  line = parsedData[i];
  charCount = (line.pass.match(new RegExp(line.letter, 'g')) || []).length || 0;
  if (charCount >= line.lower && charCount <= line.upper) ++count;
}
...
for (let i in parsedData) {
  line = parsedData[i];
  lowerPos = line.pass[line.lower - 1] == line.letter;
  upperPos = line.pass[line.upper - 1] == line.letter;
  if ((lowerPos && !upperPos) || (!lowerPos && upperPos)) ++count;
}
